...
public:
  ...
  void toggleSingle(int led) {
    ...
    digitalWrite(LED_pins[led], LED_states[led]);
  }

  void writeAll(int leds) {
    for (int led = 0; led < NUM_LEDS; ++led) {
      LED_states[led] = (leds >> led) & 1;
    }
...
private:
  static constexpr uint32_t LED_group_counts = 4;
  static constexpr uint32_t LED_pins[NUM_LEDS] = {3, 4, 5, 6, 9, 10, 11, 12, 13, 14, 15, 16};
